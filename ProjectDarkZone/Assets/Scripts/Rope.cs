...
public class Rope
{
    ...
    public void Update()
    {
        ...
        if (!foundHead)
        {
            if (!(GameObject.Find("ropeHead" + ropeNumber) == null))
            {
                head = GameObject.Find("ropeHead" + ropeNumber);
                foundHead = true;
            }
        }
        ...
        {
            GameObject.Find("ropeHead" + ropeNumber).GetComponent<Rigidbody2D>().isKinematic = true;
            //Debug.Log("Yo, I hit something!!");            
        }

    }

    void CreateRopeObject(bool firstSegment)
    {
        ...
        if (facingRight)
            rb.AddForce(new Vector2(.4f, .1f) * (1.2f * damper));
        else
        ...
        if (firstSegment)
        {
            ...
            ropeSegments[ropeSegments.IndexOf(projectile)].name = "ropeHead" + ropeNumber;
